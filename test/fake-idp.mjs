// In-process fake OpenID Provider for test/sso-e2e.mjs (Vibe Tax Research Chat).
//
...
//
// `jose` is loaded through @kisaesdevlab/vibe-auth's own dependency tree (this
// directory has no dependencies of its own).
...
// pnpm installs the package as a symlink; require() from its REAL path so jose resolves.
const require = createRequire(realpathSync(fileURLToPath(new URL('../apps/api/node_modules/@kisaesdevlab/vibe-auth/package.json', import.meta.url))));
...
export class FakeIdp {
  ...
  tokenRequests = [];
  issuerOverride;
  /** IdP session id stamped on the NEXT ID token (default `sid-<sub>`), so two
   *  logins of one user can carry different `sid`s for sid-only back-channel tests. */
  nextSid = null;
  ...
  async signIdToken(o) {
    ...
    const claims = {
      ...
      roles: u.roles,
      amr: u.amr ?? ['pwd'],
      sid: o.sid ?? this.nextSid ?? 'sid-' + u.sub,
